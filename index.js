const  XBOT = 'KARMA BOT'; // Nama Bot Whatsapp
const instagram = 'https://www.instagram.com/karmaoficial0'; // Nama Instagramlu cok
const nomer = 'https://Wa.me/+5591984155848'; // Nomor whatsapplu cok
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5591984155848:+62 823-3429-7175\n' // WhatsApp ID + phone number
            + 'END:VCARD'
...
conn.on('message-status-update', json =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @Karma`)
})
    ...
    axios.get(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ESPERE] Em andamento🔍 por favor, aguarde um momento', MessageType.text, { quoted: m })
...
axios.get(`https://st4rz.herokuapp.com/api/ytv?url=${teks}`).then((res) => {
    let hasil = `O vídeo está disponível no link abaixo, clique no link e faça o download hasilnya\n\n\nJudul: ${res.data.title}\n\nUkuran video: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
if (text.includes('#tagme')) {
 ...
 const options = {
       text: `@${nomor.split("@s.whatsapp.net")[0]} marcado!`,
...
conn.sendMessage(id, 'Deseja doar para inserir o Bot no grupo? Proprietário do bate-papo: D', MessageType.text)
}
...
axios.get(`https://api.haipbis.xyz/randomcooltext?text=${teks}`).then((res) => {
	conn.sendMessage(id, '[WAIT] Pesquisando ...🔍', MessageType.text)
    let hasil = `Sucesso de imagem de texto:) \n\n${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
    ...
    axios.get(`http://inyourdream.herokuapp.com/glitch?kata1=${teks1}&kata2=${teks2}`).then((res) => {
      imageToBase64(res.data.status)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[WAIT] Em andamento🔍 silahkan tunggu sebentar', MessageType.text, { quoted: m })
...
axios.get(`https://docs-jojo.herokuapp.com/api/samehadaku?q=${teks}`).tthen((res) => {
...
if (text.includes('#infogempa')){
    axios.get(`https://st4rz.herokuapp.com/api/infogempa`).then((res) => {
      imageToBase64(res.data.map)
        .then(
          (ress) => {
          let hasil = `*ÚLTIMAS INFORMAÇÕES SOBRE O TERREMOTO NA INDONÉSIA*\n\n*➸Centro de terremotos :* ${res.data.lokasi}\n*➸  Koordinat :* ${res.data.koordinat}\n*➸ Tempo :* ${res.data.waktu}\n*➸ Magnitude :* ${res.data.magnitude}\n*➸ Profundidade :* ${res.data.kedalaman}\n*➸ Potência :* ${res.data.potensi}`;
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[WAIT] Em andamento🔍 por favor espere um momentor', MessageType.text, { quoted: m })
            conn.sendMessage(id, buf, MessageType.image, { caption: hasil, quoted: m })
        })
    })
}
